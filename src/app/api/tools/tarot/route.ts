...
export async function POST(request: NextRequest) {
  try {
    ...
    const llmResponse = await invokeLLM({
      userId: user.id,
      systemPrompt:
        'אתה קורא טארוט מנוסה. תן פרשנות מעמיקה ב-3-4 פסקאות. התמקד בהודעה הכוללת של הפריסה. כתוב בעברית עשירה ומותאמת אישית.',
      prompt: `${questionText}הקלפים שנשלפו: ${cardNames}. פרש את הפריסה ואת הקשר בין הקלפים.`,
      maxTokens: 1000,
    })
